...
For the lower temperature of 300~K all excess volumes ($V^\text{E}$) are negative, creating more beneficial space conformation.  The lowest value is for indomethacin indicates the strongest interaction, which is in agreement with the height of RDF first peak. For all APIs, there is visible trend, with higher concentration of PLA the $V^\text{E}$ value is lower. The same trend is observed for excess energies for 3 of 4 APIs (cbz, nap, indo), for ibuprofen the trend of $E^\text{E}$ is the opposite. 
...
The RDF of hydrogen bonding with the carbonyl group in PLA (OC) is shown in Figure \ref{fig:carbonyl}. The RDF value converges to one in a long distance for all API-PLA interactions, which is in good agreement with theory. For indomethacine, the amplitude of the peak decrease with lower PLA molecules presented in the mixture. This correspond with the observation for trends in INDO-INDO interactions. The contact distance in dimer obtained from quantum optimization is 1.76 \AA, from MD simulation we obtained the value of 1.75 \AA.
...
MSD was also evaluated at a lower temperature of 300 K, Figure \ref{fig:msd_r1}. For carbamazepine, there is no visible trend of changed diffusivity for different concentrations, from that we can definitely say, that addition of PLA has minimal effect on the pure API. The situation of of naproxen is similar, we cannot determine the difference between the different concentrations in order of magnitude of simulation error. For ibuprofen, the situation is the same as for 500~K, the addition of PLA will result in lower diffusivity. For indomethacin, there is also no visible trend, the diffusivity of the mixture with the least amount of PLA is somehow higher. 
...
Since this method is sensitive to the initial state of the simulated box, more simulated data starting from different conformations must be provided to evaluate $T_\text{g}$ with the information about its uncertainty as it was described in methods section. We used data from 5 simulations from different initial states. The average value for each run and its uncertainty is given in Figure \ref{fig:results_tg}. 

TO DO: DOPOČÍTAT PRO INDO A CBZ SIMULACE


\begin{figure}[htb!]
	\centering
	\includegraphics[width=0.6\linewidth]{img/results_tg.png} 
	\caption{Calculated glass transition temperatures $T_\text{g}$ of mixtures composed from API and PLA with concentration of $x_\text{API}$=0.85 starting from 5 different conformations, mean values plotted by dashed line for each API.}
	\label{fig:results_tg}    
\end{figure}  

The representative data sets used for determination of $T_\textbf{g}$ with the hyberbolic fit are displayed in Figure \ref{fig:tg_evaluation} for each API. The data obtained correspond to the initial positions obtained from second nanosecond of the 5~ns long simulation. 
 
\begin{figure}[H]
	\centering
	\subfloat{\includegraphics[width=0.5\linewidth]{img/blank.png}}
	\subfloat{\includegraphics[width=0.5\linewidth]{img/nap_tg.png}}\\
	\subfloat{\includegraphics[width=0.5\linewidth]{img/ibu_s_tg.png}}
	\subfloat{\includegraphics[width=0.5\linewidth]{img/blank.png}} 
	\caption{Determination of the glass transition temperature $T_\text{g}$ of mixture with API concentration of $x_\text{API}$=0.85 by the non-equilibrium hyperbola fit method, second data set. Carbamazepine (\textbf{top left}), naproxen (\textbf{top right}), ibuprofen (\textbf{bottom left}) and indomethacin (\textbf{bottom right}).}
	\label{fig:tg_evaluation}    
\end{figure}

The Table \ref{tab:Tg_mix} shows the mean value of $T_\text{g}$ with the uncertainty for the mixture of API and PLA with $x_\text{API}=0.85$ obtained from 5 runs from different positions.

\begin{table}[htb!]
	\caption{Calculated glass transition temperatures $T_\text{g}$ of mixtures composed from API and PLA with concentration of $x_\text{API}$=0.85 and their standard uncertainties.}
	\centering
...
The experimental values were obtained from literature \cite for neat APIs and they are given in Table \
